...
static enum phy_mode dwc3_meson_g12a_get_id(struct dwc3_meson_g12a *priv)
{
	...
	regmap_read(priv->regmap, USB_REG_OFFSET + USB_R5, &reg);

	if (reg & USB_R5_ID_DIG_CURR)
...
static int dwc3_meson_g12a_mode_force_set(void *data, u64 val)
{
	...
	mode = val ? PHY_MODE_USB_HOST : PHY_MODE_USB_DEVICE;
...
static int dwc3_meson_g12a_otg_id_get(void *data, u64 *val)
{
	...
	*val = dwc3_meson_g12a_get_id(priv) == PHY_MODE_USB_HOST ? 1 : 0;
