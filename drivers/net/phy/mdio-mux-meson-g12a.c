...
static int g12a_ephy_pll_enable(struct clk_hw *hw)
{
	...
	return readl_poll_timeout(pll->base + ETH_PLL_CTL0, val,
				  val & PLL_CTL0_LOCK, 0, PLL_LOCK_TIMEOUT);
}
...
static void g12a_ephy_pll_init(struct device *dev)
{
	struct g12a_mdio_mux *priv = dev_get_drvdata(dev);

	writel(0x29c0040a, priv->regs + ETH_PLL_CTL0);
	writel(0x927e0000, priv->regs + ETH_PLL_CTL1);
	writel(0xac5f49e5, priv->regs + ETH_PLL_CTL2);
	writel(0x00000000, priv->regs + ETH_PLL_CTL3);
	writel(0x00000000, priv->regs + ETH_PLL_CTL4);
	writel(0x20200000, priv->regs + ETH_PLL_CTL5);
	writel(0x0000c002, priv->regs + ETH_PLL_CTL6);
