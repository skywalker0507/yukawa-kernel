...
#define TSENSOR_TRIM_CALIB_SUPPORT_MASK		GENMASK(3, 2)
#define TSENSOR_TRIM_CALIB_VALID_MASK		BIT(7)
...
struct meson_tsensor_device_data {
	const struct meson_tsensor_platform_data *param;
	const char *name;
};
...
struct meson_tsensor_data {
	int id;
	const struct meson_tsensor_device_data *data;
 ...
 */
static u32 temp_to_code(struct meson_tsensor_data *priv, int temp)
{
	const struct meson_tsensor_platform_data *param = priv->data->param;
	s64 divisor, factor, uefuse;
	u32 reg_code;

	uefuse = priv->trim_info & TSENSOR_READ_TEMP_MASK;

	factor = BIT(16) * (temp * 10 + param->B);
	factor = div_s64(factor, param->A);

	if (uefuse & TSENSOR_TRIM_SIGN_MASK)
 ...
 */
static int code_to_temp(struct meson_tsensor_data *priv, int temp_code)
{
	const struct meson_tsensor_platform_data *param = priv->data->param;
	int temp;
...
{
	struct meson_tsensor_data *priv = iio_priv(indio_dev);
	u32 reg_val;
	int ret = 0;
	int ver;

	regmap_read(priv->sec_ao_map, priv->data->param->u_efuse_off,
		    &priv->trim_info);

	ver = TSENSOR_TRIM_VERSION(priv->trim_info);

	if ((ver & TSENSOR_TRIM_CALIB_SUPPORT_MASK) == 0) {
		ret = -ERANGE;
		dev_err(&indio_dev->dev,
			"tsensor thermal calibration type not support: 0x%x!\n",
			ver);
		goto out;
	}
	...
	if ((ver & TSENSOR_TRIM_CALIB_VALID_MASK) == 0) {
		ret = -ERANGE;
		dev_err(&indio_dev->dev,
			"tsensor thermal calibration data not valid: 0x%x!\n",
			ver);
		goto out;
	}
...
static const struct meson_tsensor_platform_data meson_tsensor_g12a_cpu_param = {
	.A = 9411,
	.B = 3159,
	.m = 424,
	.n = 324,
...
static const struct meson_tsensor_platform_data meson_tsensor_g12a_ddr_param = {
	.A = 9411,
	.B = 3159,
	.m = 424,
	.n = 324,
...
static const struct meson_tsensor_device_data meson_g12a_ddr_tsensor_data = {
	.param = &meson_tsensor_g12a_ddr_param,
	.name = "meson-g12a-ddr-tsensor",
};

static const struct meson_tsensor_device_data meson_g12a_cpu_tsensor = {
	.param = &meson_tsensor_g12a_cpu_param,
	.name = "meson-g12a-cpu-tsensor",
};

static const struct of_device_id meson_tsensor_of_match[] = {
	{
		.compatible = "amlogic,meson-g12a-ddr-tsensor",
		.data = &meson_g12a_ddr_tsensor_data,
	},
	{
		.compatible = "amlogic,meson-g12a-cpu-tsensor",
		.data = &meson_g12a_cpu_tsensor,
	},
	{},
};
...
{
	struct meson_tsensor_data *priv;
...
static SIMPLE_DEV_PM_OPS(meson_tsensor_pm_ops, meson_tsensor_suspend,
